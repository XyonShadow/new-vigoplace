...
import { Box, Button, Card, CardContent, CardHeader, Divider, TextField, Container, Typography } from '@mui/material';
...
function AdminSecurity() {
  ...
  return (
    <>
       <Snackbar TransitionComponent={Slide} open={pinToast.error} autoHideDuration={6000} onClose={handleClose}>
        <Alert onClose={handleClose} severity="warning" sx={{ width: '100%' }}>
          {updatePinMutation.error?.response?.data?.message}
        </Alert>
      </Snackbar>
       <Snackbar TransitionComponent={Slide} open={pinToast.success} autoHideDuration={6000} onClose={handleClose}>
        <Alert onClose={handleClose} severity="success" sx={{ width: '100%' }}>
          {updatePinMutation?.data?.data?.message}
        </Alert>
      </Snackbar>

       <Snackbar TransitionComponent={Slide} open={passwordToast.error} autoHideDuration={6000} onClose={handleClose}>
        <Alert onClose={handleClose} severity="warning" sx={{ width: '100%' }}>
          {updatePasswordMutation.error?.response?.data?.message}
        </Alert>
      </Snackbar>
       <Snackbar TransitionComponent={Slide} open={passwordToast.success} autoHideDuration={6000} onClose={handleClose}>
        <Alert onClose={handleClose} severity="success" sx={{ width: '100%' }}>
    ...
    <Container maxWidth="lg">
      ...
      <Card>
        ...
        >
          ...
          disabled={password.oldPassword === '' || password.newPassword === '' || password.oldPassword.length <=5 || password.newPassword.length <=5}
          onClick={()=>{
...
<Card>
  ...
  >
    ...
    disabled={approval.pin === '' || approval.oldPin === '' || approval.pin.length <=5 || approval.oldPin.length <=5}
    onClick={()=>{
