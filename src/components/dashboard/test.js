...
export const UncategorizedPost = ({
  ...
  const nextSlide = () => {
    ...
    // if (currentImage.hasOwnProperty("category")) {
    //   handlePostClick(category);
    // } else {
    //   console.log("Category is not present in the current image data.");
    // }
  };

  // const handlePostClick = (category) => {
  //   const postId = data?.data[currentIndex]?.POId;
                  ...
                  <div className="2xl:w-[240px] xl:w-[240px] lg:w-[240px] md:w-[190px] w-[200px] rounded-md">
                    <ul className="space-y-5">
                      {/* {filteredResults.length > 0 && (
                        <div>
                          {selectedCategories[currentPostId]?.map(
                            (category) => (
                              <li
                                key={category.OCId}
