...
const fetchuserTransactionData = async ({ token, startMonth, endMonth, year }) => {
  if (!token) return null;

  const response = await axios.get(
    `https://api.vigoplace.com/api/admin/console/users/transactions/volume`,
    {
...
const TransactionByMonth = () => {
  ...
  const chartSeries = useMemo(
    () => [{ name: "User transaction By Months", data }],
    [data]
  );
